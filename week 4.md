...
<br>
- 2개 집단이 있을 때는 각 집단의 정규성을 조사하는 검정을 실시하고, 그 다음 t검정을 이용하게 되므로  <br>
가설 검정 작업을 반복해 버리는 검정 다중성 문제가 생김. <br>
- 데이터가 정규분포에서 얻어졌는지 여부를 조사하는 가장 좋은 없다고 해도 좋을 것임. <br> 
- 표본크기가 클 때는 아주 조금 정규분포에서 벗어난 것만으로도 정규성 검정 결과가 p<0.05가 되어, 정규 분포에서 얻은 것이 아니라고 판단되고 만다. <br>
